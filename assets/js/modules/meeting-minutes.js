...
(function(){
  ...
  const fmtDate=v=>{
    ...
    if(D&&typeof D.date==='function')return D.date(v);
    return String(v||'-');
  };
  ...
  function minutesText(item){
    return [`รายงานการประชุม: ${item.title||'-'}`,`วันที่: ${fmtDate(item.date)}`,`สถานที่: ${item.location||'-'}`,`ประธาน: ${item.chairman||'-'}`,`ผู้จด: ${item.recorderName||'-'}`,`กลุ่มงาน: ${item.department||'-'}`,'','ผู้เข้าร่วม:',item.attendees||'-','','วาระ/สาระสำคัญ:',item.agenda||'-','','สรุปผล/มติ/ข้อสั่งการ:',item.conclusion||'-',item.docUrl?'\nGoogle Doc: '+item.docUrl:'',item.pdfUrl?'PDF: '+item.pdfUrl:''].filter(Boolean).join('\n');
  }
