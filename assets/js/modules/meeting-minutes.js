...
(function(){
  ...
  window.showItMinutesQrV759=id=>{const item=((typeof itMinutesGlobal!=='undefined'?itMinutesGlobal:[])||[]).find(x=>String(x.id)===String(id));const url=item?.docUrl||item?.pdfUrl||'';if(!url)return Swal.fire('ยังไม่มีลิงก์เอกสาร','','warning');window.__itMinutesQrUrlV759=url;Swal.fire({title:'QR Code รายงานการประชุม',html:`<div class="small text-break mb-2">${esc(url)}</div><div id="itMinutesQrV759" class="d-flex justify-content-center"></div><button class="btn btn-outline-success mt-3" onclick="navigator.clipboard.writeText(window.__itMinutesQrUrlV759||'')"><i class="bi bi-link-45deg"></i> คัดลอกลิงก์</button>`,didOpen:()=>{const box=$('itMinutesQrV759');if(box&&window.QRCode)new QRCode(box,{text:url,width:220,height:220});}});};
